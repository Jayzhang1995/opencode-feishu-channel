...
在群聊中**直接发送图片**即可，无需任何特殊命令。服务会自动：

1. 通过飞书 API 下载图片
2. 调用 `tesseract` 识别中文+英文文字
3. 将识别结果作为消息内容发给 AI 处理

输出示例：
```
MSG openId=xxx type=group: (图片中识别出的文字内容)
```

> 需要系统安装 `tesseract-ocr` 及 `chi_sim`、`eng` 语言包。
> 目前支持的图片格式：PNG（飞书默认格式）。
